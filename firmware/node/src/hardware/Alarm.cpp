...
void loadThresholdFromFlash() {
  // Use RW mode so a brand-new namespace is created without the Preferences
  // library logging a misleading NOT_FOUND error for a read-only open.
  if (!alarmPrefs.begin("alarm", false)) {
    thresholdDeg = THRESHOLD_DEG;
    Serial.print("threshold: ");
    Serial.print(thresholdDeg, 2);
    Serial.println("° (NVS open failed; using default)");
    return;
  }
  bool saved = alarmPrefs.isKey("threshold");
  thresholdDeg = saved ? alarmPrefs.getFloat("threshold") : THRESHOLD_DEG;
  alarmPrefs.end();
  Serial.print("threshold: ");
  Serial.print(thresholdDeg, 2);
  Serial.println(saved ? "° (from flash)" : "° (first boot default)");
}
