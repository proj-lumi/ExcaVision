-- Request type is customer intent and must not be changed during fulfillment.
-- Repair and more-coverage requests cannot be reclassified as installations.
...
begin
  ...
  if p_path = 'repair_existing_unit' then
    ...
    if source_request.request_type not in ('Request repair', 'Request more sensors') then
      raise exception 'only service and coverage requests can add a unit at an existing site';
    end if;
    ...
    raise exception 'request type cannot be reclassified; choose an existing service destination';
  end if;
