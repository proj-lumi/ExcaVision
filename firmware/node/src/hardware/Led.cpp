...
void loadGatewayFromFlash() {
  ...
  if (!gwPrefs.begin("gateway", false)) {
    isGateway = false;
    Serial.println("gateway: OFF (NVS open failed; using safe default)");
    return;
  }
  bool saved = gwPrefs.isKey("is_gateway");
  isGateway = gwPrefs.getBool("is_gateway", false);
  gwPrefs.end();
  if (!saved) {
    Serial.println("gateway: OFF (first boot; no saved role)");
  } else {
