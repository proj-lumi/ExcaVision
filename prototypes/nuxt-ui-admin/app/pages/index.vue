...
const attention = computed(() => [
  ...data.value.requests.filter(request => request.status === 'intake').map(request => ({ label: request.site, detail: 'Installation intake needs review', to: '/requests' })),
  ...data.value.inventory.filter(node => node.state === 'Needs inspection').map(node => ({ label: node.serial, detail: 'Hardware needs inspection', to: '/inventory' }))
])
</script>
...
<template>
  <div class="admin-page">
    <div class="page-heading">
      ...
      <UButton to="/requests" icon="i-lucide-inbox" label="Open work queue" />
    </div>
    ...
    <section class="split-grid">
      ...
      <UCard>
        ...
        <template #footer><UButton to="/installations" color="neutral" variant="ghost" block label="View installations" /></template>
      </UCard>
    </section>
  </div>
</template>
