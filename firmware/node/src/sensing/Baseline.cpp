...
void loadBaselinesFromFlash() {
  // RW mode creates the empty namespace on first boot without a misleading
  // Preferences NOT_FOUND log. Baselines are still only written after capture.
  if (!prefs.begin("baselines", false)) {
    Serial.println("no saved baselines (NVS open failed)");
    return;
  }
  uint8_t loaded = 0;
  for (uint8_t i = 0; i < NUM_SENSORS; i++) {
