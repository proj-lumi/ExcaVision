...
#include "esp32-hal-uart.h"  // core 3.x: disable RX pull-up on GPIO34
#endif
...
// Slaves do not answer discovery immediately: simultaneous replies collide on
// a half-duplex bus. Each MAC gets a deterministic slot in a 256-slot window.
// This is not an address; it only spreads the one-time hello frames.
static bool helloPending = false;
static unsigned long helloAt = 0;
...
static PendingBaseline baselineQueue[NUM_SENSORS];
static uint8_t baselineQueueCount = 0;
static unsigned long baselineNextAt = 0;

static uint16_t discoverySlotForThisNode() {
  uint32_t hash = 5381;
  for (const char* p = nodeMac; *p; ++p) hash = ((hash << 5) + hash) ^ (uint8_t)*p;
  return (uint16_t)(hash & 0xFF);
}
...
static void handleLine() {
  ...
  if (isThisGateway()) {
    ...
    if (strcmp(line, "D") == 0) {
      // Schedule rather than transmit immediately. This prevents three or more
      // slaves from replying to the same broadcast at the same time.
      helloPending = true;
      helloAt = millis() + 20UL +
                discoverySlotForThisNode() * RS485_DISCOVERY_SLOT_MS;
      Serial.print("[rs485] discovery hello scheduled in ");
      Serial.print((unsigned long)(helloAt - millis()));
      Serial.println(" ms");
...
static void masterTick() {
  ...
  switch (rsState) {
    case 0:  // idle
      if (slaveCount == 0 || now - lastDiscover >= RS485_REDISCOVER_MS) {
        rs485Send("D");                 // collision-spread discovery
        rsState = 1;
        stateStart = now;
        lastDiscover = now;
...
void rs485Init() {
  ...
  // GPIO34 is input-only and has no internal pull-up. Arduino-ESP32 3.x
  // enables a UART RX pull-up by default; 2.x does not expose that setting.
#if ESP_ARDUINO_VERSION >= ESP_ARDUINO_VERSION_VAL(3, 0, 0)
  uartEnableRxInternalPull(2, false);  // Serial2 = UART2
#endif
...
void rs485SendBaseline(uint8_t channel, float bx, float by, float bz) {
  // All slaves finish a global baseline at nearly the same time. Queue the
  // B frame and send it in a MAC-derived time slot so slave replies cannot
  // collide on the half-duplex bus.
  if (baselineQueueCount >= NUM_SENSORS) {
    Serial.println("[rs485] baseline reply queue full");
    return;
  }
  baselineQueue[baselineQueueCount++] = { channel, bx, by, bz };
  if (baselineQueueCount == 1) {
    baselineNextAt = millis() + 20UL +
                     discoverySlotForThisNode() * RS485_BASELINE_SLOT_MS;
  }
}
...
void rs485Update() {
  ...
  // A slave sends its delayed discovery hello from the main-loop context.
  if (!isThisGateway() && helloPending &&
      (long)(millis() - helloAt) >= 0) {
    char h[24];
  ...
  // Send queued baseline frames one at a time. The first frame is delayed by
  // the node's MAC-derived slot; additional sensors on this node are spaced
  // apart so they also cannot overlap another node's reply.
  if (!isThisGateway() && baselineQueueCount > 0 &&
      (long)(millis() - baselineNextAt) >= 0) {
    char line[80];
    const PendingBaseline& b = baselineQueue[0];
    snprintf(line, sizeof line, "B;%s;%u;%.4f,%.4f,%.4f",
             nodeMac, b.channel, (double)b.bx, (double)b.by, (double)b.bz);
    rs485Send(line);
    for (uint8_t i = 1; i < baselineQueueCount; i++)
      baselineQueue[i - 1] = baselineQueue[i];
    baselineQueueCount--;
    if (baselineQueueCount > 0) baselineNextAt = millis() + 30UL;
  }
